...
const viewHome = async (req, res, next) => {
  try {
    if (req.session.user) {
      ...
      let bannerSlider = await bannerModel.find({ status: true }).skip(2).limit(6);
      let bannerGroup = await bannerModel
        .find({ status: true })
        .limit(2);
      const wishId = wishlist.wishlist.map(
        (wishlistItem) => wishlistItem.product_id._id
      );
